...
2) Le respect de cette exigence implique-t-il une action CONCRÈTE, SPÉCIFIQUE ET NOUVELLE des équipes — pas juste
   l'existence d'un système qualité en général ? Une ligne valable désigne UN livrable identifiable et vérifiable :
   un document PRÉCIS à obtenir/produire (avec son contenu attendu, ex: "déclaration de conformité contact
   alimentaire fournie via la spécification produit"), une analyse précise à réaliser, un ingrédient/une matière
   sèche précis à utiliser ou à proscrire, une mention précise à faire figurer sur l'étiquette, un seuil chiffré, un
   délai à tenir. Une équipe doit pouvoir cocher la ligne comme "fait" ou "pas fait" sans interprétation.
   Exclus les formulations de PRINCIPE ou de SYSTÈME QUALITÉ GÉNÉRIQUE, même si elles semblent formuler une action,
   car elles sont déjà couvertes par une certification qualité standard (IFS/BRC/ISO) et n'ajoutent rien de
   spécifique à vérifier pour CE produit/CE client. Exemples à NE PAS extraire (trop génériques) :
     - "Maintenir une procédure documentée pour la gestion des matériaux non conformes" (SOP générique de tout
       système qualité, pas propre à ce document/ce produit)
     - "Tous les matériaux d'emballage doivent être adaptés à l'usage prévu, conformes à la réglementation, sûrs et
       de qualité" (reformulation d'un principe légal général, aucun détail actionnable nouveau)
     - "Surveiller les problèmes émergents et prendre les mesures appropriées pour atténuer les risques" (obligation
       de vigilance générale sans action ni livrable précis)
   Exemple à EXTRAIRE (action et livrable précis) :
     - "Le fournisseur doit obtenir de son fournisseur de packaging une assurance écrite/déclaration de conformité,
       à fournir via la spécification produit Aldi" (document précis, nommé, avec un canal de transmission défini)
   Si la phrase est un principe générique sans traduction opérationnelle nouvelle (ex: "les produits doivent être
   sûrs et légaux", "le fournisseur doit respecter la réglementation en vigueur", "Aldi is responsible for ensuring
   controls are in place") SANS préciser QUOI faire concrètement et différemment, ce n'est pas une exigence
   actionnable — ignore-la, même si elle concerne le vin.
