...
// Doit rester synchronisé avec la contrainte CHECK sur requirements.process_stage
// (supabase/schema_v6.sql). Une valeur hors liste ne doit jamais faire
// planter l'insertion : on la ramène à null plutôt que de perdre la ligne.
const VALID_STAGES = new Set(['vrac','matieres_seches','mise_en_bouteille','analyses','logistique','administratif']);
function safeStage(s) {
  const v = (s || '').toLowerCase().trim();
  return VALID_STAGES.has(v) ? v : null;
}
...
export async function diffAndStage({ rows, origin, jurisdiction, clientId, appellationId, sourceId, documentId, sourceUrl, runId, replacesDocId }) {
  ...
  // section_id est une FK vers `sections` : si le LLM invente/déforme une
  // valeur (ex: confusion avec la liste process_stage, ou une catégorie
  // hors des 12 sections connues), un INSERT multi-lignes classique fait
  // échouer TOUT le lot (violates foreign key constraint), y compris les
  // lignes par ailleurs valides. On récupère la liste des id valides pour
  // filtrer en amont, avec une tolérance casse/espaces avant de rejeter.
  const { data: sectionsRows, error: eSec } = await db.from('sections').select('id');
  if (eSec) throw eSec;
  const validIds = new Set((sectionsRows || []).map(s => s.id));
  const normalizedMap = new Map([...validIds].map(id => [id.toLowerCase().trim(), id]));

  const currentByKey = new Map(current.map(r => [keyOf(r), r]));
  const staged = { new: 0, modified: 0, removed: 0, rejected: [] };
  ...
  for (const r of rows) {
    // Tolère une casse/espace différents (ex: "Etiquetage" -> "etiquetage"),
    // mais rejette proprement une vraie valeur inconnue plutôt que de
    // planter tout le lot sur la contrainte FK.
    const canonical = normalizedMap.get((r.section_id || '').toLowerCase().trim());
    if (!canonical) {
      staged.rejected.push({ parameter: r.parameter, section_id: r.section_id, reason: 'section_id inconnu (hors des sections définies)' });
      continue;
    }
    r.section_id = canonical;
    ...
    toInsert.push({
      ...
      process_stage: safeStage(r.process_stage),
